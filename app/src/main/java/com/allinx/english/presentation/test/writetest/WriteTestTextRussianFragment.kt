...
import com.allinx.english.extension.openFinishFromWriteTestTextRussianScreen
...
@AndroidEntryPoint
class WriteTestTextRussianFragment : Fragment() {
    ...
    private fun updateUI(){
        if(position == listWords.size){
            openFinishFromWriteTestTextRussianScreen(right,wrong)
        }
        else {
            ...
            with(binding) {
                tietWord.setText("")
                tietWord.text = null
                wordWriteTest.text = listWords[position].word
    ...
    private fun checkAnswer(answer: String){
        if (answer.equals(listWords[position].meaning, true)) {
            right += 1
        }
        else{
            wrong += 1
    ...
    companion object {
        @JvmStatic
        fun newInstance(param1: String, param2: String) = WriteTestTextRussianFragment()
    }
}