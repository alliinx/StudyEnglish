...
import com.allinx.english.databinding.FragmentWriteTestPictureBinding
...
@AndroidEntryPoint
class WriteTestPictureFragment : Fragment() {

    private lateinit var binding: FragmentWriteTestPictureBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentWriteTestPictureBinding.inflate(inflater,container,false)
        return binding.root
    }
    ...
    private fun updateUI(){
        if(position == listWords.size){
            openFinishFromWriteTestPictureScreen(right,wrong)
        }
        else {
            ...
            with(binding) {
                tietWord.setText("")
                tietWord.text = null
                imgWriteTestWord.load(listWords[position].image)
    ...
    companion object {
        @JvmStatic
        fun newInstance() = WriteTestPictureFragment()
    }
}